...
test.describe("UCL Fantasy Scout E2E", () => {
  ...
  test("complete squad analysis flow with web screenshot", async ({ page }) => {
    ...
    const imagePath = path.join(process.cwd(), 'sample_data', 'Ucl-fantasy-web.png');
    ...
    const analyzeButton = page.locator('button:has-text("Analyze Squad"), button:has-text("Analyse Squad")');
    ...
    try {
      await page.waitForSelector('text=/Matchday Confirmation/i', { timeout: 10000 });
      ...
      // Click submit button
      const submitButton = page.locator('button').filter({ hasText: /submit|confirm|continue|ok/i }).first();
